...
void searchFunction(Info* information, char* query) {
    ...
    for (int i = 0; i < information->count; i++) { // Compare query and content
        if (strstr(information->list[i].name, query) != NULL || strstr(information->list[i].phone, query) != NULL ||
            strstr(information->list[i].memo, query) != NULL) {
            mvprintw(match + 6, 1, "%d %s %s %s", match + 1, information->list[i].name, information->list[i].phone, information->list[i].memo);
            match++;
        }
    }

    if (match != 0) {
        mvprintw(match + 7, 1, "Match found. Press any key to continue.");
    }
    else {
        mvprintw(match + 7, 1, "No match found. Press any key to continue.");
    }
...
void addFunction(Info* information, char* name, char* phone, char* memo) {
    ...
    if (c == 'Y' || c == 'y') { // Add information
        ...
        information->count++;
        mvprintw(13, 1, "Information added successfully. Press any key to continue.");
    }
    else {
        mvprintw(13, 1, "Add operation canceled. Press any key to continue.");
    }
...
void deleteFunction(Info* information, char* name) {
    ...
    for (int i = 0; i < information->count; i++) { // Compare name/query and content
        if (strstr(information->list[i].name, query) != NULL || strstr(information->list[i].phone, query) != NULL ||
            strstr(information->list[i].memo, query) != NULL) {
            mvprintw(match + 5, 1, "%d %s %s %s", match + 1, information->list[i].name, information->list[i].phone, information->list[i].memo);
            match++;
        }
    }

    if (match == 0) {
        mvprintw(match + 5, 1, "No match found. Press any key to continue.");
        refresh();
        getch();
        return;
    }
    ...
    if (index < 1 || index > match) {
        mvprintw(match + 8, 1, "Invalid index. Press any key to continue.");
        refresh();
        getch();
        return;
    }

    for (int i = 0; i < information->count; i++) { // Find the information location you want
        if (strstr(information->list[i].name, query) != NULL || strstr(information->list[i].phone, query) != NULL ||
            strstr(information->list[i].memo, query) != NULL) {
            deletionCount++;
        }
    ...
    mvprintw(match + 8, 1, "Information deleted successfully. Press any key to continue.");
    refresh();
    getch();
}
...
void saveFunction(Info* information) {
    ...
    for (int i = 0; i < information->count; i++) { //Save infor line by line
...
void loadFunction(Info* information) {
    ...
    while (fgets(line, sizeof(line), file) != NULL) { //Get infor
        sscanf(line, "%29[^:]:%19[^:]:%39[^\n]", name, phone, memo);
        if (information->count < MAXIMUM) {
            strcpy(information->list[information->count].name, name);
            strcpy(information->list[information->count].phone, phone);
            strcpy(information->list[information->count].memo, memo);
            information->count++;
            ...
            clear();
            refresh();
            mvprintw(1, 1, "The maximum limit has been reached. Cannot load more information. Press any key to continue.");
            refresh();
            getch();
            break;
        }
...
int main(int argc, char* argv[]) {
    ...
    LoadInform(&information); //data.txt
